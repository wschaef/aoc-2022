...
const solution: any = (input: string) => {
    ...
    const starts = board.flat().filter(it => it.value === 1)
    const results = starts.map(start => solveForStart(start, end, board))
    return Math.min(...results)
}

function solveForStart(start: Point, end: Point, board: Point[][]): number {
    ...
    return endState ? endState.steps : Number.MAX_VALUE
}
...
new Solver(solution, 'src/day12/input.txt', 2).print()  